...
int main()
{
    ...
    try
    {
        while (true)
        {
            ...
            SOCKET  sS;
            if ((sS = socket(AF_INET, SOCK_DGRAM, NULL)) == INVALID_SOCKET)//При создании сокета используется параметр SOCK_DGRAM в TCP используется SOCK_STREAM 
